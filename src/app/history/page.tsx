...
const problems: ProblemItem[] = [
  {
    ...
    desc: "Geçmiş taramalarda performans etkisi oluşturan alan",
    icon: "speed",
    tone: "red",
    solution:
  ...
  },
  {
    ...
    desc: "Security Basics modülünde kontrol edilir",
    icon: "shield",
    tone: "orange",
    solution:
  ...
  },
  {
    ...
    desc: "Görsel QA ve performans skorunu etkiler",
    icon: "image",
    tone: "yellow",
    solution:
...
export default function HistoryPage() {
    ...
    return [
      {
        title: "Toplam Tarama",
        value: String(total),
        desc: "Veritabanındaki tüm kayıtlar",
        icon: "chart",
        tone: "blue",
      },
      {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      const matchesDate = matchDateFilter(row.createdAtMs, dateFilter);
  ...
  }, [rows, searchTerm, statusFilter, scoreFilter, criticalOnly, dateFilter]);
  ...
  const selectedScoreLabel =
    scoreFilters.find((item) => item.value === scoreFilter)?.label ??
    "Tüm Skorlar";
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      ...
      try {
        ...
        const normalizedRows = normalizeHistoryRows(data.scans ?? []);
        ...
        const message =
          error instanceof Error
            ? error.message
            : "Analiz geçmişi alınırken bilinmeyen bir hata oluştu.";
  ...
  const deleteSelectedRow = async () => {
    ...
    try {
      const response = await fetch(
        `/api/scans/${encodeURIComponent(deleteRow.id)}`,
        {
          method: "DELETE",
        },
      );
      ...
      if (!response.ok) {
        throw new Error(data.message ?? "Tarama silinemedi.");
      }
      ...
      const message =
        error instanceof Error ? error.message : "Tarama silinemedi.";
          ...
          <div className="mx-auto grid w-full max-w-[1600px] gap-6 px-4 py-6 sm:px-6 lg:py-8 xl:grid-cols-[1fr_300px]">
            <div className="min-w-0">
              ...
              {(isHistoryLoading || historyError) && (
                ...
                >
                  {historyError || "Analiz geçmişi yükleniyor..."}
                </div>
              )}
                    ...
                    <tbody>
                      {paginatedRows.map((row) => (
                        ...
                        >
                          ...
                          <td className="px-5 py-5 text-[14px] font-bold text-[#b8c0d0]">
                            {row.date}
                            <br />
                            <span className="text-[12px] text-[#9aa4b7]">
                              {row.time}
                            </span>
                          </td>
                          <td className="px-5 py-5">
                      ...
                      {!paginatedRows.length && (
                        <tr>
                          <td
                            colSpan={6}
                            className="px-5 py-10 text-center text-[14px] font-bold text-[#9aa4b7]"
                          >
                            {isHistoryLoading
                  ...
                  <p className="text-[13px] font-bold text-[#aeb6c8]">
                    {filteredRows.length
                      ? `${startIndex + 1}-${Math.min(
                          ...
                          filteredRows.length,
                        )} / ${filteredRows.length} tarama gösteriliyor`
      ...
      {selectedRow && (
        <Modal onClose={() => setSelectedRow(null)} maxWidth="max-w-[620px]">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            ...
            <DetailBox
              label="Tarih"
              value={`${selectedRow.date} ${selectedRow.time}`}
            />
...
function RightPanel({
  ...
  const averageScore = scoredRows.length
    ? Math.round(
        scoredRows.reduce((total, row) => total + Number(row.score), 0) /
          scoredRows.length,
      )
    : 0;
      ...
      <section className="rounded-xl border border-white/9 bg-[#0d1423]/88 p-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
        ...
        <div className="mt-7 border-t border-white/8 pt-5">
          ...
          <div className="mt-3 flex items-center justify-between text-[11px] font-extrabold text-[#9ba5b8]">
            ...
            <span>{Math.max(0, 58 - completedRows.length)} tarama kaldı</span>
          </div>
        </div>
      </section>
...
function SummaryLine({
  ...
  return (
    <div className="flex items-center justify-between text-[15px] font-extrabold">
      ...
      <span className={green ? "text-[#25d18c]" : "text-[#dce2ef]"}>
        {value}
      </span>
    </div>
  );
...
function normalizeHistoryRows(scans: ApiScanItem[]): HistoryRow[] {
  return scans.map((scan) => {
    const type = getHistoryRowType(scan);
    ...
    return {
      ...
      url: scan.url,
      date: formatHistoryDate(new Date(createdAtMs)),
      time: formatHistoryTime(new Date(createdAtMs)),
      score,
      status: getHistoryStatusLabel(scan.status, type),
      critical:
        scan.status === "completed" || scan.status === "running"
          ? String(criticalCount)
          : "-",
      type,
      icon: getHistoryIcon(scan.url),
      createdAtMs,
    };
  });
}

function getHistoryRowType(scan: ApiScanItem): RowType {
...
function getHistoryStatusLabel(status: string, type: RowType) {
...
function getHistoryIcon(url: string) {
  const value = url.toLowerCase();

  if (value.includes("shop") || value.includes("store")) return "cart";
  if (value.includes("blog") || value.includes("haber")) return "article";
  if (value.includes("landing")) return "window";
